...
(default OFF, mirroring the executable flag below). Its Xpress backend is
always compiled in and needs no Xpress installation at build time: at
runtime, OR-Tools loads `$XPRESSDIR/lib/libxprs.so` dynamically, so the
machine running `SOLVERCH=6` needs a licensed Xpress and `XPRESSDIR`.
```
$> mkdir build
$> mkdir build/external
$> cd build/external
$> cmake ../../external -DCMAKE_BUILD_TYPE=<BUILD_TYPE_3PARTIES> [-DUSE_ORTOOLS=ON]
$> cmake --build .
```
...
- `USE_XPRESS` (default = OFF, requires `USE_ORTOOLS=ON`): authorize `SOLVERCH=6` at runtime. Pure authorization switch; Xpress availability is decided on the running machine (`XPRESSDIR` + license).
